...
version was not already published as different bytes, and — for a public build —
that every distributable EXE is signed by an approved publisher and verifies under
`signtool verify /pa /tw`. Anything short of that **fails closed**: the pipeline
stops and nothing official is over-written.
...
`npm run package:win` (public) / `npm run package:win:qa` (dev):

1. **verify source** — `verify:release`.
2. **build** — `build` / `build:qa`.
...
6. **sign** — `sign-release.mjs` signs every distributable EXE and verifies each
   ...
   leaves them unsigned. No certificate is assumed; public fails closed without one.
...
| `payload-binding-unproven` | Installer↔payload containment not proven (public). |
| `lock-integrity-unattested` | No clean-install / lockfile-integrity attestation (public). |
| `unsigned-public` / `untrusted-timestamp-public` / `publisher-not-approved` | Public EXE unsigned, untimestamped, or signed by a non-allowlisted publisher. |

Public-required evidence gates: `packaged-e2e`, `approval`, `shared-state`,
`thin-installer`, `telegram`. qa requires the first three and may leave the last
two as honest external blockers.

## Inputs supplied out-of-band (absent ⇒ public fails closed)
