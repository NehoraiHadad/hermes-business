...
- התקנה פעילה במחשב הפיתוח: Hermes Agent `0.19.1` (`2026.7.30`).
- release רשמי חדש ביותר שנמצא בטווח התאימות: `0.20.1` (`v2026.8.13`).
- טווח התאימות הנתמך: `>=0.19.0 <0.21.0`.
- מעטפת: `0.4.0-alpha.4`.
- Windows 11 x64, התקנת native תחת `%LOCALAPPDATA%\hermes`.

חוזה ה־Desktop Plugin וכל משטחי השליחה שה־WhatsApp policy plugin עוטף נבדקו
מול קוד המקור הבלתי־משתנה של `v2026.8.13`. בדיקות ה־E2E החיות המלאות עדיין
רצות על ההתקנה הפעילה `0.19.1`, ולכן הרחבת הטווח אינה מוצגת כהוכחת E2E ל־0.20.1.
...
בשימוש העסקי שני הממשקים משתמשים באותו Hermes Home וב־Profile `default`. אין Runtime,
...
`>=0.19.0 <0.21.0`, הגרסה המאומתת `0.20.1`, ומפת ה־pins). כל שכבה מטבעה מחזיקה עותק
...
1. **Preflight לפני כל מוטציה — לפני עצירה או גיבוי.** תחילה **gate של שיטת
   ...
   והעדכון מבוטל אם היעד חורג מהטווח הנתמך `>=0.19.0 <0.21.0`. כל אלה קורים
