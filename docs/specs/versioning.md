...
- `scanReleases(releases, currentVersion)` — סורק את הרשימה ומחזיר `{ eligible, examined, undecided }`. `eligible` = הגבוהה ביותר לפי `parseSemver(tag_name)`. `examined` = כמה entries נקראו בפועל (מפקד ריק הוא **עובדה אחרת** ממפקד שנקרא ולא נמצא בו דבר חדש). `undecided` = כמה entries **לא ניתן היה לסדר כלל** מול הגרסה הנוכחית. ההבחנה הזו היא הליבה של הסמנטיקה:
  - **הדרה החלטית (לא נספרת כ־undecided)** — `draft: true` (לא פורסם כלל, ולכן לעולם אינו עדכון זמין), ו־prerelease כשהנוכחית יציבה (הדרה מכוח **מדיניות ערוץ**, D1 — לא מחוסר ידיעה). הדרה כזו נבדקת **לפני** הפירסור, כדי שתג פגום של prerelease מודר לא "יזהם" את שלמות הסריקה.
  - **הדרה בלתי־מוכרעת (נספרת)** — entry שאינו אובייקט, או `tag_name` בלתי־פרסיבי על release אמיתי, מפורסם, בערוץ. הוא מדולג ואינו מפיל (כבעבר), אך **אסור לטעון שהוא ישן יותר** מהגרסה הרצה.
- `selectEligibleRelease(releases, currentVersion)` — חצי־המועמד של `scanReleases` (נשמר כ־export נפרד; "מי מנצחת" היא שאלה שראוי לבדוק בפני עצמה).
- `linkHeaderHasNextPage(linkHeader)` — טהור: האם כותרת `Link` של התשובה (RFC 8288) מכריזה על עמוד `rel="next"`. זהו **סיגנל השלמות היחיד** — ראו `isCompleteScan` ו־§8. הפרסור בוחן רק את חלק ה־params של כל segment, כך שמחרוזת `rel="next"` **בתוך** URI לעולם לא תיחשב ליחס אמיתי.
- `isCompleteScan({ truncated, undecided })` — סריקה נחשבת **מפקד שלם** רק בהוכחה חיובית בשני הצירים: `truncated === false` (הכותרות היו קריאות **וגם** לא נשאה קישור לעמוד הבא) **וגם** `undecided === 0`. שדה חסר אינו הוכחה. שלמות **אינה** אומרת דבר על ריקנות — רשימה ריקה היא מפקד שלם לחלוטין (ראינו את כל אפס ה־entries שלה); השאלה אם אפס entries יכולים לשאת פסק־דין נענית ב־`decideVerdict`.
- `decideVerdict(current, eligible, scan)` — מחזיר בדיוק אחד מ:

| status | תנאי | חובת הוכחה |
|---|---|---|
| `update-available` | `eligible > current` | fetch תקין + tag פרסיבי |
| `up-to-date` | `eligible == current`, **או** אין מועמד כלל **וגם** `isCompleteScan(scan)` **וגם** `examined > 0` | חיובית בלבד |
| `dev-ahead` | `current` גדולה מכל מה שפורסם (מועמד נמצא, והוא נמוך ממנה) | fetch תקין |
| `unknown` | כל דבר אחר: timeout, HTTP != 200, rate-limit, JSON פגום, גרסה רצה בלתי־פרסיבית, **סריקה חלקית ללא מועמד**, או **מפקד ריק** (`examined === 0`) | — ברירת המחדל |

**מדוע "סריקה שלמה ולא־ריקה ללא מועמד" היא `up-to-date` ולא `unknown`:** זו בדיוק הטענה ש־`מעודכן` מוסרת למשתמש — "לא פורסם עבורך שום דבר חדש יותר ממה שאתה מריץ" — והיא **הוכחה חיובית מלאה** כשה־fetch הצליח, מפקד אמיתי נקרא, והוא היה שלם (המקרה החי: התקנה יציבה מול repo שכל ה־releases שלו הם prereleases). דיווח `unknown` שם הוא שקר לכיוון ההפוך: הוא מסתיר מצב תקין־ומוכח מאחורי הודעת כשל מזויפת, זהה לזו של נפילת רשת.

**ומדוע מפקד ריק (`[]`) הוא בכל זאת `unknown`:** תשובה ריקה היא **חסרת תוכן**, ולקרוא לתוכה הרגעה זו בדיוק ההפרה שה־§1.4 מגדיר (`מעודכן` ללא בדיקה). לריפו הזה יש releases שפורסמו ו־ledger שלעולם אינו מתכווץ (`scripts/lib/release/prior-ledger.mjs`), ולכן `[]` הוא **אנומליה** — תקלה upstream, proxy, mirror שגוי — ולא מצב יציב וכן של "עוד לא שוחרר דבר". בין `up-to-date` (עמיד: נשמר ב־cache וב־`lastStatus`, ובולע בשקט עדכון ממתין) לבין `unknown` (בר־התאוששות: "לא ניתן לבדוק"), fail-closed בערוץ עדכון רגיש־אבטחה פירושו `unknown`. גם מקרה ה־bootstrap — לפני ה־release הראשון בכלל — נופל לכאן: מי שמריץ build שלא פורסם הוא מפתח, ועבורו "לא ידוע" אינו פחות אמיתי מ־`מעודכן`. שימו לב שזו בדיקת **אי־ריקנות ולא בדיקת שלמות**: מפקד ריק הוא שלם לחלוטין, ולכן התנאי יושב ב־`decideVerdict` ולא ב־`isCompleteScan`. (`dev-ahead` שגוי כאן מסיבה נוספת: הוא טוען על **סדר** מול releases שפורסמו, ובלי אף אחד כזה אין מול מה להיות "מקדימה".)

**מדוע קטיעה (truncation) אינה פוסלת פסק־דין כשנמצא מועמד:** GitHub מחזיר releases לפי סדר יצירה יורד, ולכן כל מה שעמוד קטוע משמיט **נוצר לפני** כל מה שראינו; יחד עם דוקטרינת הגרסאות המונוטונית של הריפו (§5.2 + ledger אי־השימוש־החוזר, §1.3) הזנב המושמט ישן ממש. לכן השוואה **המעוגנת ב־release אמיתי שנמצא בעמוד שכן נקרא** נשארת מכריעה תחת קטיעה — לשלושת הפסקים `update-available` / `up-to-date` / `dev-ahead`. (`update-available` בטוח גם בלי טיעון המונוטוניות: קיומו של משהו חדש יותר הוכח, ו־entries ישנים שהושמטו אינם יכולים לבטל אותו.) **ללא מועמד אין עוגן כלל** — ולכן דווקא הענף הזה, ורק הוא, דורש מפקד שלם.
...
- `checkCompanionUpdate({ force })` — GET אחד:
  ...
  - **בקשה אחת בלבד — ללא מעקב אחרי pagination.** מכיוון ש־`per_page=20` עלול לקטוע את הרשימה, השכבה האימפורית קוראת את כותרת ה־`Link` של התשובה ומדווחת `{ releases, truncated }`. `truncated: false` נקבע **רק** בהוכחה חיובית: הכותרות היו קריאות **וגם** אין בהן `rel="next"`. אובייקט headers חסר/לא־קריא **נכשל סגור** ל־`truncated: true` (לא הצלחנו להסתכל ⇒ אסור לטעון שראינו הכול). המשמעות של ערך הכותרת מוכרעת בטהור (`linkHeaderHasNextPage`); השכבה האימפורית רק קוראת אותה.
...
- **הטיימר חוזר וממתין את עצמו (re-arming setTimeout), לא one-shot ולא `setInterval`**: תכל'ס יושב ב־tray ולא נסגר ב־`window-all-closed`, ולכן סשן חי שבועות; one-shot יחיד אחרי `ready` היה מבצע בדיקה אחת בלבד לכל אורך הסשן. `setInterval` אינו פתרון — Windows משהה טיימרים ב־sleep/hibernate, ומחזור ארוך עלול לירות מיד עם היקיצה ואז לסטות. אחרי כל הרצה הטיימר נדרך מחדש מתוך המצב העמיד, כך שיקיצה מוקדמת או מאוחרת משנה רק *מתי נשאלת השאלה*, לא את התשובה: ה־throttle העמיד הוא שמכריע אם בכלל נעשית פנייה לרשת.
- **חזרה של המשתמש מה־tray** (`show`/`restore`/`focus` של החלון) מפעילה את אותו מסלול פסיבי בדיוק — לעולם לא בדיקה כפויה — כך שמשתמש שחזר אחרי ימים מקבל תשובה מיד, וכל עוד ה־throttle בתוקף אין שום פנייה לרשת. פרץ אירועים מכווץ לבדיקה אחת.
- ההכרעה עצמה (האם לבדוק עכשיו ומתי להתעורר) היא **פונקציה טהורה** ב־`electron/companion-update-schedule.cjs` (`decidePassiveCheck`) עם בדיקות יחידה משלה; ב־`main.cjs` נשאר רק החיווט הלא־טהור (טיימר אמיתי, חלון אמיתי, push ב־IPC), באותו פיצול טהור/לא־טהור של `companion-update-core.cjs` מול `companion-update.cjs`. חותמת עמידה שנמצאת ב**עתיד** (שעון שהוזז אחורה) אינה נחשבת "טרייה" — היא גוררת בדיקה אחת, אחרת הפיצ'ר היה נתקע לצמיתות.
...
- כאשר `update-available`: בלוק מתחת לשורה — `גרסה <latest> זמינה` + תאריך + notes מקוטעים כטקסט, ואז **אחד משני מסלולים** (ראו §7.3):
  - `managedUpdate === true` ⇒ כפתור ראשי **`עדכן עכשיו`** (עדכון מנוהל בתוך האפליקציה).
  - `managedUpdate === false` ⇒ כפתור `פתח דף הורדה` (⇒ `openExternal(downloadUrl)`) + הערה קבועה: `ההורדה נפתחת בדפדפן. הקובץ אינו חתום — Windows עשוי להציג אזהרה. מומלץ לאמת SHA-256 מול SHA256SUMS.txt שבדף ההורדה. סגרו את תכל'ס לפני הרצת ההתקנה.`

  המסלול הידני **לעולם אינו נמחק** — הוא נשאר נגיש גם אחרי כשל בעדכון המנוהל.
...
### 7.3 עדכון בלחיצה אחת — שתי הסכמות נפרדות

המסלול המנוהל מחליף את ההורדה הידנית בכפתור אחד, אבל **לא** הופך את העדכון לאוטומטי. שתי פעולות נפרדות, כל אחת ביוזמת המשתמש:

1. **`עדכן עכשיו`** ⇒ `downloadCompanionUpdate()`. מוריד ומאמת בלבד — שום דבר לא מותקן. מד התקדמות דטרמיניסטי כש־`totalBytes` הוא מספר, ו**בלתי־דטרמיניסטי** כשהוא `null` (אין להמציא מכנה). ניתן לבטל בכל רגע.
2. **`התקן והפעל מחדש`** ⇒ `applyCompanionUpdate()`. מופיע רק אחרי שההורדה אומתה. הטקסט אומר במפורש מה עומד לקרות: תכל'ס ייסגר, ההתקנה תרוץ ברקע, והאפליקציה תעלה מחדש — כדקה. העוזר לא זמין בינתיים.

הפרדת ההסכמות היא העיקר: הורדה מאומתת שממתינה אינה מתקינה את עצמה, גם לא בהפעלה הבאה. הורדה שאומתה בהפעלה קודמת חוזרת כהצעה (`companionUpdateState()` בעת mount), לא כפעולה.

**קלט מהרנדרר: אין.** שתי הפעולות אינן מקבלות פרמטרים כלל — לא URL, לא נתיב, לא גרסה. כל אופרנד נגזר ב־main מתוצרים ש־main עצמו ייצר (ה־verdict, ה־journal העמיד). ראו `electron/ipc-companion-update.cjs`.

**כשל** מוצג עם ה־`message` של main מילה במילה (עברית, בטוח למשתמש, ומצהיר שלא בוצע שינוי) — הרנדרר לא ממציא נוסח משלו לכשל ש־main כבר תיאר.

**מצב תקוע**: `applied-unhealthy` / `unexpected-version` אינם מנקים את ה־journal בכוונה (המצב באמת לא הוכרע), ולכן ההודעה עשויה לחזור בכל הפעלה. המסך מציע אישור־קריאה מקומי כדי שזו לא תהיה נדנוד בלתי־ניתן לסגירה — בלי שום IPC שמוחק journal.

### 7.4 מחזור חיי מפתח החתימה — ראשי ורזרבה

`UPDATE_TRUST_KEYS` ב־`electron/update-trust.cjs` היא **מפה**, ומאז 2026-08-18 היא מכילה **שני** מפתחות:

| תפקיד | key id | חצי פרטי | מתי חותם |
|---|---|---|---|
| ראשי (Primary) | `…947e2bb83d384c67` | `%USERPROFILE%\.tachles-release\update-signing-key.pem` | כל שחרור שגרתי |
| רזרבה (Reserve) | `…c6379a37ef1fb417` | מנותק, מחוץ למכונת הבנייה | **לעולם לא** — רק אחרי אובדן/דליפה של הראשי |

**למה רזרבה חייבת להישלח מראש.** החלפת מפתח מגיעה רק להתקנות **עתידיות**. אפליקציה שכבר יושבת אצל משתמש סומכת בדיוק על המזהים שהודרו לתוכה ביום הבנייה, ואין לנו ערוץ להוסיף לה מזהה — כל ערוץ כזה היה בדיוק הדלת שהחתימה נועדה לנעול. לכן מפתח שנוצר **אחרי** שהבנייה יצאה חסר ערך לאותה התקנה, ואובדן המפתח הראשי היה מחייב כל משתמש להתקין מחדש ידנית. הרזרבה היא מסלול השחזור היחיד, והרגע היחיד שבו אפשר להקים אותה בזול הוא **לפני** שיש בסיס משתמשים.

**מה זה קונה ומה לא — במפורש.** מפני **אובדן** ההגנה מלאה ובלתי־מותנית. מפני **דליפה** ההגנה תקפה רק במידה ששני החצאים הפרטיים שוכנים במקומות שונים: אם הרזרבה נשארת על מכונת הבנייה ליד הראשי, פריצה אחת לוקחת את שניהם והרזרבה לא קנתה דבר. `resolveSigningKey` מחפש אך ורק את הנתיב הראשי המדויק, ולכן הרזרבה לעולם לא נבחרת בטעות — שימוש בה דורש `--key` מפורש או `TACHLES_UPDATE_SIGNING_KEY`.

**אין ביטול — פער מוצהר.** הוספת הרזרבה מאפשרת לנו לחתום שוב; היא **אינה** מסוגלת לגרום להתקנות קיימות להפסיק לסמוך על מפתח ראשי גנוב. כל עוד הפיילוט קטן זו חשיפה מקובלת ומודעת. עם בסיס משתמשים אמיתי יידרש מנגנון נפרד (רצפת גרסה מינימלית חתומה, או רשימת מזהים פסולים) — ולא הוא ולא מסלול ההפצה שלו קיימים היום.

**נוהל שימוש ברזרבה** (אירוע חד־פעמי, לא שגרה): לחתום עם `--key <נתיב הרזרבה>`, לשחרר, ובאותה גרסה לייצר מפתח ראשי חדש ולהוסיף אותו למפה **לצד** הרזרבה. מרגע שהרזרבה שימשה בפועל היא מפתח פעיל לכל דבר, ולכן יש להקים רזרבה חדשה מיד — אחרת חוזרים למצב של מפתח יחיד בלי מסלול חזרה.

### 7.5 חזרה לגרסה הקודמת (rollback)

עדכון שנחת ושבר משהו השאיר עד כה את המשתמש בלי דרך חזרה מלבד התקנה ידנית. `חזרו לגרסה הקודמת` סוגר את זה בצעד **אחד** אחורה, בהסכמה נפרדת, ותוך שימוש חוזר כמעט מלא במסלול העדכון הקדימה.

**מדוע זה בטוח, למרות שזה בדיוק מה שהבקרה המרכזית אוסרת.** הבקרה של המסלול קדימה היא ש־manifest חייב להיות **חדש ממש** מהמותקן — כך נחסמת התקפת downgrade שבה מראה עוין מגיש manifest אותנטי אך ישן. עוגן ההגנה מפני replay הוא `expectedVersion`, והוא **נשאר זהה בדיוק** גם כאן: ה־manifest חייב להתאים לו במדויק. מה שמשתנה הוא רק **מהיכן הערך מגיע** — ביומן העמיד שלנו, הגרסה שההתקנה הזו רשמה שממנה עדכנה, שנכתבה לפני שהמתקין רץ. היעד הוא עובדה על העבר של המכונה, לא טענה שמישהו ברשת יכול להשפיע עליה. התוקף החזק ביותר, ששולט לגמרי בפיד, יכול להגיש בדיוק את הגרסה שהמשתמש הריץ אתמול — זו שהוא מבקש בשמה.

מכאן נגזרות המגבלות, וכולן נאכפות ב־`companion-rollback-core.cjs`:

| כלל | נימוק |
|---|---|
| צעד **אחד** אחורה בלבד; אין רשימת גרסאות לבחירה | כל יעד נוסף הוא יעד שהתוקף לא נדרש להרוויח |
| העדכון הרשום חייב להיות על הגרסה **הפועלת עכשיו** | אחרת היומן מתאר התקנה אחרת ואינו מוכיח דבר |
| היעד חייב להיות **ישן ממש** | גם גורם לכך שההצעה נעלמת מעצמה אחרי rollback מוצלח |
| רק תוצאות `applied` / `applied-unhealthy` הן עוגן | `apply-failed` לא שינה גרסה, `unexpected-version` מצהיר שלא הוכח דבר |
| הרנדרר אינו מעביר **שום** פרמטר | מי שיכול לנקוב בגרסה יכול לנקוב בכל גרסה |

**מה משותף למסלול קדימה** (ולכן לא שוכפל): מנוע ההורדה, סדר `אימות ההצהרה → גיבוב הבתים`, ה־journal, ההתאוששות בהפעלה (שמכריעה בהשוואת גרסה בלבד ולכן אדישה לכיוון), ו־`applyCompanionUpdate` על אותו argv של NSIS. אומת מול תבניות `app-builder-lib`: **אין בהן שום השוואת גרסאות**, ולכן התקנת build ישן מעל חדש היא התקנה רגילה. ההבדל היחיד הוא `direction: 'rollback'` שמהפך את כלל הסדר — ורק אותו.

**מלכודת שנסגרה במפורש**: כשההצעה נשענת על רשומה **פעילה** בשלב `applying` (מצב `applied-unhealthy` — בדיוק המקרה שבו הכי צריך לחזור), `beginCompanionUpdate` היה דורס אותה בתחילת ההורדה. אילו ההורדה הייתה נכשלת, היומן היה מתנקה כ־`failed`, ההיסטוריה הייתה נותרת בלי עוגן, והמשתמש היה נשאר על גרסה שבורה **בלי כפתור חזרה**. לכן הרשומה מארכבת קודם כ־`applied-unhealthy`. כישלון בארכוב אינו עוצר את החזרה — הוא עולה רק את יכולת הניסיון החוזר.

**מה זה לא**: לא שחזור נתונים ולא חזרה אוטומטית. ההגדרות והנתונים נשארים במקומם, וייתכן שפריטים שנוספו בגרסה החדשה לא יופיעו בישנה — וזה נאמר בממשק.
...
| הגרסה הרצה (`app.getVersion()`) בלתי־פרסיבית | `unknown` | כנ"ל — אין מול מה להשוות, גם על סריקה שלמה |
| fetch תקין, אין מועמד, ו**הסריקה חלקית**: כותרת `Link` עם `rel="next"` (רשימה קטועה), או headers לא־קריאים, או entry מפורסם־בערוץ שה־tag שלו בלתי־פרסיבי (`undecided > 0`) | `unknown` | כנ"ל — "לא ראינו הכול" **אינו** הוכחת עדכניות |
| **רשימה ריקה** (`examined === 0`), גם כשהסריקה שלמה | `unknown` | כנ"ל — תשובה חסרת תוכן **אינה** הוכחת עדכניות. `[]` בריפו הזה הוא אנומליה (§6.1), ו־`מעודכן` עליה היה בולע עדכון ממתין בשקט ובאופן עמיד |
| fetch תקין, **מפקד לא־ריק ושלם** (`examined > 0` **וגם** `truncated === false` **וגם** `undecided === 0`) שלא הניב מועמד — למשל התקנה יציבה מול repo שכל ה־releases שלו prereleases | `up-to-date` | `מעודכן` — הוכחה חיובית מלאה שלא פורסם דבר חדש יותר. **לא** הודעת כשל |
| fetch תקין, `eligible == current` | `up-to-date` | `מעודכן` |
| fetch תקין, `current` מעל המועמד שנמצא | `dev-ahead` | `גרסה מקומית חדשה מהפורסם` |
| רשימה קטועה **אך נמצא מועמד** | הפסק הרגיל מההשוואה (`update-available` / `up-to-date` / `dev-ahead`) | כרגיל — ההשוואה מעוגנת ב־release אמיתי, והזנב המושמט נוצר לפניו (§6.1) |
...
עקרון משלים, באותה חומרה: **`unknown` פירושו "לא הצלחנו לקבוע", ולא "לא מצאנו".** בדיקה שהצליחה במלואה, קראה מפקד אמיתי והוכיחה שאין מה להתקין — חייבת לומר זאת. שתי הטעויות סימטריות: טענת עדכניות ללא הוכחה היא fail-open, ודיווח כשל על בדיקה שהצליחה הוא שקר שמסתיר מצב מוכח־תקין מאחורי אותה הודעה בדיוק של נפילת רשת.

הגבול בין השתיים עובר בשלוש עובדות נפרדות שאסור למזג: **קטיעה** (`truncated`), **אי־הכרעה** (`undecided`) ו**ריקנות** (`examined`). שתי הראשונות אומרות "לא ראינו הכול"; השלישית אומרת "לא היה מה לראות". כל אחת מהן לבדה מספיקה כדי לשלול `up-to-date` בענף חסר־המועמד, וכולן נדרשות בהוכחה חיובית ונכשלות סגור.
...
> **עדכון 2026-08-18 — §10 אינו מתאר עוד את המצב בפועל.** מאז מומש **עדכון מנוהל בלחיצה אחת ללא תעודת חתימה** (§7.3): האפליקציה מורידה בעצמה, מאמתת, ומריצה התקנה שקטה. הניתוח שלהלן נשאר תקף במלואו לגבי **electron-updater** — הוא עדיין לא מחווט, `build.publish` עדיין `null`, ו־`verify-no-update-metadata.mjs` עדיין נשאר על כנו ללא שינוי (המסלול המנוהל אינו מייצר `latest.yml`, ולכן ההגנה מ־§1.2 עדיין אמיתית ולא הוסרה).
>
> מה שהשתנה הוא **עוגן האמון**, לא ההחלטה על electron-updater. במקום Authenticode שאין, נחתמת הצהרה נפרדת לכל release:
> - זוג מפתחות **Ed25519**; הציבורי נארז באפליקציה (`electron/update-trust.cjs`), הפרטי חי מחוץ למאגר.
> - `update-manifest.json` — asset חתום שמצהיר על `{version, installer.sha256, installer.bytes, installer.name}`, מפורסם בעסקה האטומית של שאר ה־sidecars (`finalize-release.mjs`).
> - בזמן ריצה: מאמתים תחילה את **ההצהרה** (חתימה + התאמת גרסה כהגנה מפני replay/downgrade), ורק אחר כך מורידים ומשווים את ה־SHA-256 של **הבייטים**. אף אחד מהשלבים אינו מספיק לבדו, והסדר הוא תכונת האבטחה.
>
> מה שעוגן זה **לא** נותן, ורק תעודה תיתן: Authenticode, ביטול אזהרת SmartScreen בהתקנה הראשונה מהדפדפן, וההגנה המובנית של electron-updater. לכן §10.2 נשאר יעד פתוח — אבל הוא כבר לא **חוסם** עדכון בלחיצה אחת.
...
- `companion-update-core.test.ts`: כל שורת §8 כ־case; סדר semver מלא (כולל `0.4.0 > 0.4.0-alpha.9`, `alpha.2 < alpha.10` מספרית); סינון prerelease ליציבה; tags פגומים מדולגים; `sanitizeReleaseNotes` (קיטום, תווי בקרה, אי־פירוש markdown); אימות prefix של `downloadUrl` (כולל `https://github.com.evil.tld/...` ⇒ נדחה). **סמנטיקת השלמות**: `scanReleases` מבדיל הדרה החלטית (draft / prerelease־במדיניות, כולל tag פגום שלהם) מהדרה בלתי־מוכרעת (`undecided`); `linkHeaderHasNextPage` על `rel="next"` מצוטט/חשוף/רשימה, ו־`rel="next"` **בתוך** URI ⇒ `false`; `isCompleteScan` דורש `truncated === false` **וגם** `undecided === 0` ושדה חסר אינו הוכחה; `decideVerdict` — מפקד שלם ולא־ריק ללא מועמד ⇒ `up-to-date`, סריקה קטועה ללא מועמד ⇒ `unknown`, **רשימה ריקה ⇒ `unknown` גם כשהסריקה שלמה**, ומועמד שנמצא נשאר מכריע גם תחת קטיעה.
- `companion-update.test.ts` עם fetch מוזרק (דפוס ה־deps של `hermes-update-flow.test.ts`): timeout ⇒ unknown; 403 ⇒ unknown; serial guard; throttle/cache; כתיבת state אטומית. **המקרה החי**: התקנה יציבה `0.4.0` מול רשימה שכולה prereleases ללא כותרת `Link` ⇒ `up-to-date` (ו־`lastStatus` העמיד רושם זאת), אותה רשימה עם `rel="next"` או עם headers לא־קריאים ⇒ `unknown`, `[]` ⇒ `unknown`, ואותה רשימה עבור התקנת alpha ⇒ `update-available` (ערוץ האלפא לא הופרע).
